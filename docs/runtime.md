...
CLI `inspect` counts tensor shapes without allocating a model or opening tokenizer/portable-package assets. Its `auto` backend uses the training selector. Memory categories are disjoint estimates, not measured peaks; an artificial cap cannot certify physical fit, and unavailable capacity readings produce `UNKNOWN` unless the cap already disproves fit. MPS capacity subtracts current driver allocation from the configured fraction of its recommendation and also respects available system RAM.

Staging remains incomplete: `stage --through smoke|warmup` records those stages without executing an isolated model pilot. Use an explicit target backend and a short real training run to exercise forward/backward/checkpointing; neither an estimate nor a stage label proves measured fit or throughput. The [beginner guide](from-toy-to-useful.md#5-a-smaller-instruction-learner-before-the-100m-run) supplies that workflow.
