...
MLX uses the same immutable generation, artifact, provenance, promotion, recovery, and evidence contract. Its native codec stores validated JSON and optimizer safetensors. Same-engine continuation restores native state; cross-engine promotion copies canonical weights and starts fresh training state. Native files can be verified without the MLX SDK; execution still requires it. See [checkpointing](checkpointing.md#native-mlx-generations).

Both engines validate at step zero, each configured cadence, and the terminal update. Exact supervised-target counts, finite loss, protocol settings, and artifact identities accompany each checkpoint-bound report. Non-multiple budgets mask only the remaining valid targets; wholly masked chunks do not update state. Validation selects nonempty blocks before applying its batch limit and binds supervision/byte-address files when present. Training and validation modes are restored; diagnostics are captured before validation can replace them.
