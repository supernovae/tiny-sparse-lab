...
The [`lexical-engram` lesson](research/lesson-paths.md) provides a standalone final-injection configuration and initialized shape/diagnostic probe. `memory_injection` supports final and embedding placement; these are the declared alternatives in the [placement study](research/engram-placement.md), not evidence for middle/multiple injection or semantic retrieval. Lookup diagnostics remain mechanism observations, not score or transfer proof.
