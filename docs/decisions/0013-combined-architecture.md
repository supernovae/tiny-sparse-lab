...
Provide a runnable configuration that combines MLA attention, local top-1 MoE feed-forward routing, and causal byte-address memory in one decoder. Each mechanism retains its own contract: MLA controls attention representation, MoE chooses one local FFN expert per token, and byte memory receives prepared causal addresses after final normalization.
