...
Phase 1 does not implement semantic retrieval, training, attachment, or model integration. Encoder identities are producer declarations, not an independent certification that two encoders share meaningful coordinates. A correctly verified pack is an immutable, portable artifact—not evidence of inference or training compute savings.
