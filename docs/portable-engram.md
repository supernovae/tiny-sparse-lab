...
The original portable lexical-table path does not implement semantic retrieval, training, attachment, or model integration. Phase D adds a separate verified semantic EngramPack retriever and direct DenseLM adapter API; see [Verified semantic memory](research/semantic-memory.md). Encoder identities remain producer declarations, not independent certification that two encoders share meaningful coordinates. A verified pack is an immutable portable artifact—not evidence of inference or training compute savings.
